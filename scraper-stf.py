...
def scrapAcordaoPage(url):
    ...
    document = {'_id': int(doc_id), 'url': url}
...
@click.option('--maxpg', default=0, help='Quantidade máxima de páginas de busca a serem analisadas.')
...
def scrap(termo, base, espera, maxpg):
    global db
    global client
    global timeout
    ...
    while page_num <= last_page:
        ...
        if page_num == 1:
            ...
            # Quando maxpg é nãp-positivo, processar até a última página.
            if maxpg < 1:
                match = re.search(DOC_COUNT_RE, remove_tags(resp.text))
                if match:
                    doc_count = match.group(1).replace('.','')
                    doc_count = int(doc_count)
                    last_page = math.ceil(doc_count/BATCH_SIZE)
